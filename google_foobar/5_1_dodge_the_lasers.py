...
def solution(s):
    def solve(n):
        ...
        n_prime = int(math.floor((A - 1) * n))
        ret = n*n_prime + n*(n+1)//2 - n_prime*(n_prime + 1)//2 - solve(n_prime)
        return int(ret)
    
    return str(solve(long(s)))


def test(s):
    n = long(s)
    ret = sum([math.floor(i * A) for i in range(1, n+1)])
    return str(long(ret))

assert solution('77') == '4208'
assert solution('77') == test('77')
assert solution('5') == '19'
for i in range(1, 300):
    s = str(i)
    if solution(s) != test(s):
        print(i, solution(s), test(s))
